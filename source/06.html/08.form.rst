...
* Un documento |HTML| podría tener asociados distintos aspectos visuales (temas
  ...
  cambio del archivo |CSS| activo). Para ello, no se necesita el concurso de
...
**form** (:devmoz:`form`)
   ...
   .. table:: Atributos de form
      ...
      |              |            | atendiendo a los criteros que tenga              |
...
**input** (:devmoz:`input`)
   ...
   .. table:: Atributos comunes de input
      ...
      +--------------+----------+-----------------------------------------------+
      | autofocus    | \-       | Atrae hacia a sí el foco. Por tanto, debería  |
      ...
      +--------------+----------+-----------------------------------------------+
      | pattern\ [#]_| regexp   | Expresión regular con la que debe concordar   |
...
la relación en el compartamiento: pinchar sobre la etiqueta equivale a
...
.. note:: Es posible incluir dentro del elemento |datalist| un elemento
   |select|. No tiene efecto alguno, pero si el navegador es algo antiguo y no
   soporta listas de sugerencias, las sugerencias  se verán como una lista
   desplegable; y con ayuda de código *Javascript* podremos hacer algún apaño
   para que el usuario siga teniendo acceso a las sugerencias.

...
**style** (:devmoz:`style`)
   ...
   .. note:: Para referir estilo |CSS| que se encuentre en fichero aparte debe
      usarse :code:`<link>`, que :ref:`se explica un poco más adelante <html-link>`.
...
**meta** (:devmoz:`meta`)
   ...
   es que se distintgan unos de otros a través del atributo :code:`name`. Sin
   embargo, hay una excepción a ello muy importante: la definición de la
   codificación del documento::

      <meta charset="UTF-8">

   que es recomendable incluir siempre, aunque no sea obligatorio. Otra elemento *meta*
   que no utiliza :code:`name` es el que contiene el atributo
   `http-equiv <https://developer.mozilla.org/en-US/docs/Web/HTML/Element/meta#http-equiv>`_,
   que permite definir para el navegador valores de campos de cabecera |HTTP|
   ausentes.

   De entre los metadatos que podemos declarar haciendo uso del atributo
   :code:`name` distinguermos:
   ...
   Metadatos sobre **campos** de cabecera:
      Se utiliza el atributo :code:`http-equiv`, en vez de :code:`name`. Algunas
      posibilidades son:
      ...
      * Evita el *cacheo* de la página por el navegador::

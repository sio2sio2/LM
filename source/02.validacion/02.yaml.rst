...
.. note:: Obviamente, si se pretende definir una gramática de |YAML| con |JSON|\
   ...
   un |JSON|. Por ejemplo, no podremos usar ``undefined`` o todas las claves
   tendrán que ser cadenas, a pesar de que |YAML| no tiene esas limitaciones.
...
* El todo terreno `Visual Studio Code`_, que sirve para validar tanto |JSON|
  ...
  |YAML|
   ...
   extensión |YAML| en, y que el |JSON| de configuración no es exactamente
   igual:

...
etc.). Las últimas versiones de `Javascript
<https://developer.mozilla.org/es/docs/Web/JavaScript>`_, que fue un lenguaje
...
Hay tres versiones de *XPath*:

.. rst-class:: simple

* La versión **1**, que apareció en el año 1999, y es universalmente
  soportada. Es la versión que soporta `libxml <http://xmlsoft.org/>`_, la
  librería que utiliza :command:`xmlstarlet`.

* La versión **2**, que apareció en 2007, e introduce mejoras muy
  significativas. No todo el software soporta esta especificación.

  .. seealso:: Una exhaustiva comparación entre las versiones **1.0** y **2.0**
     se encuentra `en este documento
     <http://www.mulberrytech.com/papers/XPath-2-0-User-Grp-HTML/contents.html>`_.

* La  versión **3**, que apareció en 2014. En el mundo del software libre
  soportan esta versión la librería `Saxon
  ...
  versión para `C++ <https://es.wikipedia.org/wiki/C%2B%2B>`_ y puede ser usada
  a través del commando de línea `xqilla
  <http://xqilla.sourceforge.net/HomePage>`_. Aunque tiene paquete en *Debian*,
  desgraciadamente no está disponible para *Bookworm*, por `problemas de
  compilación con g++-11
  <https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=984418>`_.

  .. note:: `xidel <http://www.videlibri.de/xidel.html>`_, también software
     libre, permite el uso de *XPath* **3.0** para la extracción de datos.

* La versión **3.1**, publicada en 2017, que añade soporte para consultar
  también documentos |JSON|.

.. caution:: Estos apuntes desarrollan *XPath* **1.0**. 
...
Practicando con *XPath*
***********************
Para practicar lo aprendido con *XPath* puede usarse :command:`xmlstarlet`
tanto para obtener información del |XML| original, como para modificarlo.

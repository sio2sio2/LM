...
su aspecto guarda bastante parecido con ellos.

Se usa muy habitualmente en la transmisión de datos entre servidor y cliente
web, en lo que ha desplazado casi por completo a |XML|.
...
#. :dfn:`Secuencia`  (o :ref:`array <json-array>`), que es un dato constituido
   ...
   que provoca que anidación. Por ejemplo:
...
Sabido esto, las reglas básicas son:
...
Esquemas
--------
Hasta ahora los nodos que hemos incluido en nuestros ejemplos som nodos
sin etiquetar o, siendo más precisos, eran nodos etiquetados implícitamente,
puesto que las *etiquetas* permiten definir de qué tipo es el nodo. Esto supone
que el procesador deduce el tipo de dato dependiendo de cómo se hubiera escrito.
Por ejemplo:
...
* La clave "num" es una cadena.
* El valor asociado a "num" es un entero.
* La clave "cadena" es una cadena.
* El valor asociado a "cadena" es una cadena.
...
profundizaremos en la *etiquetas* bajo el próximo, para centrarnos ahora en qué
es un esquema:
...
**Core schema** (o sea, esquema básico)
   ...
   esquema |JSON| y si escribimos alguna variante como :code:`NULL` se entenderá
   una cadena.  Sin embargo, en este esquema también puede escribirse este dato
   como :code:`Null`, :code:`NULL` o dejar el valor vacío.
...
Además de estos tipos incluidos en el esquema básico, existen otros  que suelen
implementar los procesadores:

...
.. note:: El documento puede validarse con :download:`este DTD
   <files/casilleros.dtd>`.

.. note:: El documento no es un texto marcado, sino una sucesión de datos, por
...
**Entidad**
   ...
   La entidad :code:`&EFI;` está definida en fichero aparte (ya se verá cómo se
...
Validez
-------
Cuando se analiza la validez de un documento |XML|, podemos establecer dos
niveles de validez:

- El cumplimiento de las **reglas generales** que todo |XML| debe cumplir.
  Cuando un documento lo alcanza, se dice que el documento está :dfn:`bien
  formado`.

- El cumplimiento de unas **reglas gramaticales precisas** definidas en un |DTD|
  (o en un documento equivalente como un :ref:`XSD <xsd>` o un :ref:`Relax-NG
  <rnc>`). Cuando el documento alcanza este nivel, se le denomina :dfn:`valido`.
  Por supuesto, todo |XML| válido es bien formado. 

Volviendo a\ :ref:`l ejemplo <xml-ejemplo>`, para comprobar si está bien formado
nos bastará descargar exclusivamente el |XML| y utlizar un validador como
:program:`xmlstarlet`::

   $ xmlstarlet val -e casilleros.xml

Ahora bien, si queremos concluir que es válido, tendremos que tomar también el
|DTD| que se proporciona justamente en la nota inferior.

.. seealso:: Para más información sobre :program:`xmlstarlet` consulte el
   :ref:`epígrafe sobre validación con un DTD <dtd>`.
   ...
   * Las entidades pueden ser de dos tipos:
     ...
     a. **Predefinidas**: Hay cinco entidades que predefine la
        ...
        * :code:`&#DEC;` si el código es decimal.
        ...
        directamente con el teclado. El modo escribir este carácter mediante una
